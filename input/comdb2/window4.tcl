  ...
  SELECT a, max(c) OVER (ORDER BY a ROWS BETWEEN 1 PRECEDING AND 1 PRECEDING)  FROM t5;
  SELECT a, max(c) OVER (ORDER BY a ROWS BETWEEN 0 PRECEDING AND 0 PRECEDING)  FROM t5;
  SELECT a, max(c) OVER (ORDER BY a ROWS BETWEEN 2 FOLLOWING AND 1 FOLLOWING)  FROM t5;
  SELECT a, max(c) OVER (ORDER BY a ROWS BETWEEN 1 FOLLOWING AND 1 FOLLOWING)  FROM t5;
  SELECT a, max(c) OVER (ORDER BY a ROWS BETWEEN 0 FOLLOWING AND 0 FOLLOWING)  FROM t5;
  ...
  SELECT lead(y) OVER win FROM t1  WINDOW win AS (ORDER BY x);
  SELECT lead(y, 2) OVER win FROM t1  WINDOW win AS (ORDER BY x);
  SELECT lead(y, 3, -1) OVER win FROM t1  WINDOW win AS (ORDER BY x);
  SELECT     lead(y) OVER win, lead(y) OVER win  FROM t1  WINDOW win AS (ORDER BY x);
  SELECT     lead(y) OVER win,     lead(y, 2) OVER win,     lead(y, 3, -1) OVER win  FROM t1  WINDOW win AS (ORDER BY x);
  DROP TABLE IF EXISTS t1;
  ...
  INSERT INTO t7(id, a, b) VALUES    (1, 1, 2), (2, 1, NULL), (3, 1, 4),    (4, 3, NULL), (5, 3, 8), (6, 3, 1);
