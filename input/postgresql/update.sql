 ...
 -- Make even numbers odd, or vice versa return NEW;
 END $$ LANGUAGE plpgsql;
 ...
 \d+ part_def insert into range_parted values ('c', 9);
 ...
 insert into utrtest values (2, 'bar') returning *, tableoid::regclass, xmin = pg_current_xact_id()::xid as xmin_ok;
  -- fails insert into utrtest values (2, 'bar') returning *, tableoid::regclass;
  ...
  -- fails  update utrtest set a = 3 - a from (values (1), (2)) s(x) where a = s.x returning *, tableoid::regclass;
 ...
 -- This is changing partition key column. return NEW;
 END $$ LANGUAGE plpgsql;
