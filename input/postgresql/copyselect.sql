create table test1 (id serial, t text);
 ...
 insert into test2 (t) values ('E');
  create view v_test1 as select 'v_'||t from test1;
 ...
 select 1/0;
    -- row, then error select 1/0\;
 copy (select 1) to stdout;
 -- error only copy (select 1) to stdout\;
 copy (select 2) to stdout\;
 select 0\;
 select 3;
 -- 1 2 3  create table test3 (c int);
 select 0\;
 copy test3 from stdin\;
 copy test3 from stdin\;
 select 1;
 -- 1 1 \. 2 \. select * from test3;
 drop table test3;
