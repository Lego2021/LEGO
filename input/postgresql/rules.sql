 ...
 ** Remember the delete rule on rtest_v1: It says ** DO INSTEAD DELETE FROM rtest_t1 WHERE a = old.a ** So this time both rows with a = 2 must get deleted \p \r delete from rtest_v1 where b = 12;
 ...
 create rule rules_foorule as on insert to rules_foo where f1 < 100 do instead insert into rules_foo2 values (new.f1);
 ...
 drop table cchild;
  ...
  SELECT tablename, rulename, definition FROM pg_rules WHERE schemaname IN ('pg_catalog', 'public') ORDER BY tablename, rulename;
 ...
 select xmin, * from rules_fooview;
  -- fail, views don't have such a column  select reltoastrelid, relkind, relfrozenxid from pg_class where oid = 'rules_fooview'::regclass;
  drop view rules_fooview;
  create table rules_fooview (x int, y text);
 ...
 \d+ rules_src  create rule r4 as on insert to rules_src do instead insert into rules_log AS trgt SELECT NEW.* RETURNING trgt.f1, trgt.f2;
 create rule r5 as on update to rules_src do instead UPDATE rules_log AS trgt SET tag = 'updated' WHERE trgt.f1 = new.f1;
 \d+ rules_src  create table rule_t1(f1 int, f2 int);
 ...
 \d+ rule_t1 drop table rule_t1, rule_dest;
 ...
 SELECT * FROM rule_v1;
  \d+ rule_v1  ALTER RULE InsertRule ON rule_v1 RENAME TO NewInsertRule;
 ...
 -- ON SELECT rule cannot be renamed  DROP VIEW rule_v1;
 DROP TABLE rule_t1;
  create view rule_v1 as values(1,2);
 \d+ rule_v1 alter table rule_v1 rename column column2 to q2;
 ...
 SELECT * FROM hat_data WHERE hat_name IN ('h8', 'h9', 'h7') ORDER BY hat_name;
  ...
  drop table hats;
 drop table hat_data;
  CREATE FUNCTION func_with_set_params() RETURNS integer AS 'select 1;
' LANGUAGE SQL SET search_path TO PG_CATALOG SET extra_float_digits TO 2 SET work_mem TO '4MB' SET datestyle to iso, mdy SET local_preload_libraries TO "Mixed/Case", 'c:/''a"/path', '', '0123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789' IMMUTABLE STRICT;
