CREATE TABLE mvtest_t (id int NOT NULL PRIMARY KEY, type text NOT NULL, amt numeric NOT NULL);
 ...
 CREATE INDEX mvtest_aa ON mvtest_bb (grandtot);
  \d+ mvtest_tvm \d+ mvtest_tvm \d+ mvtest_tvvm \d+ mvtest_bb  CREATE SCHEMA mvtest_mvschema;
 ALTER MATERIALIZED VIEW mvtest_tvm SET SCHEMA mvtest_mvschema;
 \d+ mvtest_tvm \d+ mvtest_tvmm SET search_path = mvtest_mvschema, public;
 \d+ mvtest_tvm  INSERT INTO mvtest_t VALUES (6, 'z', 13);
  SELECT * FROM mvtest_tm ORDER BY type;
 ...
 \d+ mvtest_vt2 CREATE MATERIALIZED VIEW mv_test2 AS SELECT moo, 2*moo FROM mvtest_vt2 UNION ALL SELECT moo, 3*moo FROM mvtest_vt2;
 \d+ mv_test2 CREATE MATERIALIZED VIEW mv_test3 AS SELECT * FROM mv_test2 WHERE moo = 12345;
 ...
 -- error CREATE MATERIALIZED VIEW mvtest_mv_v (ii, jj) AS SELECT i, j FROM mvtest_v;
 -- ok CREATE MATERIALIZED VIEW mvtest_mv_v_2 (ii) AS SELECT i, j FROM mvtest_v;
 ...
 -- ok ALTER TABLE mvtest_v RENAME COLUMN i TO x;
 ...
 \d+ mv_unspecified_types SELECT * FROM mv_unspecified_types;
 ...
 refresh materialized view mvtest_error;
  -- fail here drop materialized view mvtest_error;
 ...
 -- error CREATE MATERIALIZED VIEW IF NOT EXISTS matview_ine_tab AS SELECT 1 / 0;
 ...
 -- ok EXPLAIN (ANALYZE, COSTS OFF, SUMMARY OFF, TIMING OFF) CREATE MATERIALIZED VIEW matview_ine_tab AS SELECT 1 / 0;
 -- error EXPLAIN (ANALYZE, COSTS OFF, SUMMARY OFF, TIMING OFF) CREATE MATERIALIZED VIEW IF NOT EXISTS matview_ine_tab AS SELECT 1 / 0;
 -- ok EXPLAIN (ANALYZE, COSTS OFF, SUMMARY OFF, TIMING OFF) CREATE MATERIALIZED VIEW matview_ine_tab AS SELECT 1 / 0 WITH NO DATA;
 -- error EXPLAIN (ANALYZE, COSTS OFF, SUMMARY OFF, TIMING OFF) CREATE MATERIALIZED VIEW IF NOT EXISTS matview_ine_tab AS SELECT 1 / 0 WITH NO DATA;
 -- ok DROP MATERIALIZED VIEW matview_ine_tab;
