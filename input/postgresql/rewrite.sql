SET synchronous_commit = on;
  ...
  CREATE FUNCTION exec(text) returns void language plpgsql volatile AS $f$ BEGIN EXECUTE $1;
 END;
 $f$;
 CREATE ROLE regress_justforcomments NOLOGIN;
  SELECT exec( format($outer$CREATE FUNCTION iamalongfunction() RETURNS TEXT IMMUTABLE LANGUAGE SQL AS $f$SELECT text %L$f$$outer$, (SELECT repeat(string_agg(to_char(g.i, 'FM0000'), ''), 50) FROM generate_series(1, 500) g(i))));
 SELECT exec( format($outer$COMMENT ON FUNCTION iamalongfunction() IS %L$outer$, iamalongfunction()));
 SELECT exec( format($outer$COMMENT ON ROLE REGRESS_JUSTFORCOMMENTS IS %L$outer$, iamalongfunction()));
 ...
 VACUUM FULL pg_shdescription;
 VACUUM FULL iamalargetable;
