SET max_parallel_maintenance_workers = 0;
 ...
 CLUSTER abbrev_abort_uuids USING abbrev_abort_uuids__abort_increasing_idx;
  SELECT id, abort_increasing, abort_decreasing, noabort_increasing, noabort_decreasing FROM abbrev_abort_uuids ORDER BY ctid LIMIT 5;
  SELECT id, abort_increasing, abort_decreasing, noabort_increasing, noabort_decreasing FROM abbrev_abort_uuids ORDER BY ctid DESC LIMIT 5;
 ROLLBACK;
  BEGIN;
 SET LOCAL enable_indexscan = false;
 ...
 SET LOCAL enable_material = off;
  SELECT $$ SELECT col12, count(distinct a.col1), count(distinct a.col2), count(distinct b.col1), count(distinct b.col2), count(*) FROM test_mark_restore a JOIN test_mark_restore b USING(col12) GROUP BY 1 HAVING count(*) > 1 ORDER BY 2 DESC, 1 DESC, 3 DESC, 4 DESC, 5 DESC, 6 DESC LIMIT 10 $$ AS qry \gset  EXPLAIN (COSTS OFF) :qry;
 :qry;
  SET LOCAL work_mem = '100kB';
