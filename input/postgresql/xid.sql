 ...
 select snap from snapshot_test order by nr;
  ...
  select id, pg_visible_in_snapshot(id::text::xid8, snap) from snapshot_test, generate_series(11, 21) id where nr = 2;
  select id, pg_visible_in_snapshot(id::text::xid8, snap) from snapshot_test, generate_series(90, 160) id where nr = 4;
 ...
 COMMIT;
  BEGIN;
 SELECT pg_current_xact_id() AS committed \gset COMMIT;
  BEGIN;
 SELECT pg_current_xact_id() AS rolledback \gset ROLLBACK;
  BEGIN;
 SELECT pg_current_xact_id() AS inprogress \gset  SELECT pg_xact_status(:committed::text::xid8) AS committed;
 ...
 -- BootstrapTransactionId is always committed SELECT pg_xact_status('2'::xid8);
 -- FrozenTransactionId is always committed SELECT pg_xact_status('3'::xid8);
 -- in regress testing FirstNormalTransactionId will always be behind oldestXmin  COMMIT;
  BEGIN;
 CREATE FUNCTION test_future_xid_status(xid8) RETURNS void LANGUAGE plpgsql AS $$ BEGIN PERFORM pg_xact_status($1);
