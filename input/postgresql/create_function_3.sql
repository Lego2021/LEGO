 ...
 CREATE FUNCTION functest_A_2(text[]) RETURNS int LANGUAGE 'sql' AS 'SELECT $1[1]::int';
 ...
 ALTER FUNCTION functest_B_3(int) COST 100;
    -- unrelated change, no effect SELECT proname, provolatile FROM pg_proc WHERE oid in ('functest_B_1'::regproc, 'functest_B_2'::regproc, 'functest_B_3'::regproc, 'functest_B_4'::regproc) ORDER BY proname;
 ...
 SELECT proname, proleakproof FROM pg_proc WHERE oid in ('functest_E_1'::regproc, 'functest_E_2'::regproc) ORDER BY proname;
  ALTER FUNCTION functest_E_1(int) LEAKPROOF;
 ALTER FUNCTION functest_E_2(int) STABLE;
    -- unrelated change, no effect SELECT proname, proleakproof FROM pg_proc WHERE oid in ('functest_E_1'::regproc, 'functest_E_2'::regproc) ORDER BY proname;
  ALTER FUNCTION functest_E_2(int) NOT LEAKPROOF;
    -- remove leakproof attribute SELECT proname, proleakproof FROM pg_proc WHERE oid in ('functest_E_1'::regproc, 'functest_E_2'::regproc) ORDER BY proname;
 ...
 SELECT pg_get_functiondef('functest_F_2'::regproc);
   CREATE FUNCTION functest_S_1(a text, b date) RETURNS boolean LANGUAGE SQL RETURN a = 'abcd' AND b > '2001-01-01';
 ...
 END;
  CREATE FUNCTION functest_S_10(a text, b date) RETURNS boolean LANGUAGE SQL BEGIN ATOMIC SELECT a = 'abcd' AND b > '2001-01-01';
 END;
 ...
 END;
  ...
  CREATE FUNCTION functest_S_xx(x anyarray) RETURNS anyelement LANGUAGE SQL RETURN x[1];
  CREATE FUNCTION functest_S_xx(x date) RETURNS boolean LANGUAGE SQL RETURN x > 1;
  CREATE FUNCTION functest_S_15(x int) RETURNS boolean LANGUAGE SQL BEGIN ATOMIC select case when x % 2 = 0 then true else false end;
 END;
 ...
 CREATE FUNCTION functest_IS_5(x int DEFAULT nextval('functest1')) RETURNS int LANGUAGE SQL AS 'SELECT x';
  CREATE FUNCTION functest_IS_6() RETURNS int LANGUAGE SQL RETURN nextval('functest1');
  CREATE TABLE functest2 (a int, b int);
  CREATE FUNCTION functest_IS_7() RETURNS int LANGUAGE SQL RETURN (SELECT count(a) FROM functest2);
 ...
 DROP FUNCTION functest_b_1;
  -- error, not found DROP FUNCTION functest_b_2;
  -- error, ambiguous    CREATE FUNCTION functest1(a int) RETURNS int LANGUAGE SQL AS 'SELECT $1';
 ...
 DROP FUNCTION functest1(a int);
    CREATE FUNCTION functest_sri1() RETURNS SETOF int LANGUAGE SQL STABLE AS ' VALUES (1), (2), (3);
 ';
  SELECT * FROM functest_sri1();
 EXPLAIN (verbose, costs off) SELECT * FROM functest_sri1();
  CREATE FUNCTION functest_sri2() RETURNS SETOF int LANGUAGE SQL STABLE BEGIN ATOMIC VALUES (1), (2), (3);
 END;
