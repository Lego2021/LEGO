  ...
  select '(Joe,)'::fullname;
    -- ok, null 2nd column select '(Joe)'::fullname;
    -- bad select '(Joe,,)'::fullname;
    -- bad select '[]'::fullname;
          -- bad select ' (Joe,Blow)  '::fullname;
  -- ok, extra whitespace select '(Joe,Blow) /'::fullname;
  -- bad  create temp table quadtable(f1 int, q quad);
  insert into quadtable values (1, ((3.3,4.4),(5.5,6.6)));
 insert into quadtable values (2, ((null,4.4),(5.5,6.6)));
  select * from quadtable;
  select f1, q.c1 from quadtable;
        -- fails, q is a table reference  select f1, (q).c1, (qq.q).c1.i from quadtable qq;
  ...
  -- error, type mismatch  select * from quadtable;
 ...
 -- the NULL is not examined select ROW(11,'ABC') < ROW(11,'DEF') as true;
 ...
 select ROW(1,2) in (ROW(3,4), ROW(1,2::int8));
  ...
  select thousand, tenthous, four from tenk1 where (thousand, tenthous, four) > (998, 5000, 3) order by thousand, tenthous;
  explain (costs off) select thousand, tenthous from tenk1 where (998, 5000) < (thousand, tenthous) order by thousand, tenthous;
  select thousand, tenthous from tenk1 where (998, 5000) < (thousand, tenthous) order by thousand, tenthous;
 ...
 select (r).f3 from (select row(1, 2.0) as r) ss;
  -- fail select (r).* from (select row(1, 2.0) as r) ss;
  select ROW();
 select ROW() IS NULL;
 select ROW() = ROW();
 ...
 -- fail, but should complain about cantcompare   create type testtype1 as (a int, b int);
 ...
 select row(1, -3)::testtype1 *> row(1, -2)::testtype1;
  ...
  -- byval different sizes select row(1, true)::testtype2 *< row(2, true)::testtype2;
 select row(-2, true)::testtype2 *< row(-1, true)::testtype2;
 select row(0, false)::testtype2 *< row(0, true)::testtype2;
 select row(0, false)::testtype2 *<> row(0, true)::testtype2;
  create type testtype3 as (a int, b text);
  -- variable length select row(1, 'abc')::testtype3 *< row(1, 'abd')::testtype3;
 ...
 select row(1, 'abc')::testtype3 *<> row(1, 'abd')::testtype3;
  ...
  -- by ref, fixed length select row(1, '(1,2)')::testtype4 *< row(1, '(1,3)')::testtype4;
 ...
 select text(fullname) from fullname;
  -- error select fullname.text from fullname;
  -- error select cast (row('Jim', 'Beam') as text);
 select (row('Jim', 'Beam'))::text;
 select text(row('Jim', 'Beam'));
  -- error select (row('Jim', 'Beam')).text;
  -- error  insert into fullname values ('Joe', 'Blow');
