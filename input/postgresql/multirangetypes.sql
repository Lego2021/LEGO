 ...
 SELECT   room_id, range_agg(booked_during) FROM     reservations GROUP BY room_id ORDER BY room_id;
  SELECT  range_agg(r) FROM    (VALUES ('[a,c]'::textrange), ('[b,b]'::textrange), ('[c,f]'::textrange), ('[g,h)'::textrange), ('[h,j)'::textrange) ) t(r);
 ...
 create type textrange2 as range(subtype=text, multirange_type_name=_textrange1, collation="C");
  ...
  -- should fail select multirange_of_text(textrange1('a','Z')) @> 'b'::text;
 select _textrange1(textrange2('a','z')) @> 'b'::text;
  drop type textrange1;
 drop type textrange2;
   create function anyarray_anymultirange_func(a anyarray, r anymultirange) returns anyelement as 'select $1[1] + lower($2);
' language sql;
  ...
  create function bogus_func(anyelement) returns anymultirange as 'select int4multirange(int4range(1,10))' language sql;
  create function bogus_func(int) returns anymultirange as 'select int4multirange(int4range(1,10))' language sql;
  create function range_add_bounds(anymultirange) returns anyelement as 'select lower($1) + upper($1)' language sql;
 ...
 select multirangetypes_sql(nummultirange(numrange(1,10)), ARRAY[2,20]);
  -- match failure  create function anycompatiblearray_anycompatiblemultirange_func(a anycompatiblearray, mr anycompatiblemultirange) returns anycompatible as 'select $1[1] + lower($2);
' language sql;
  ...
  create function anycompatiblerange_anycompatiblemultirange_func(r anycompatiblerange, mr anycompatiblemultirange) returns anycompatible as 'select lower($1) + lower($2);
' language sql;
 ...
 select arraymultirange(arrayrange(ARRAY[2,1], ARRAY[1,2]));
  -- fail  select array[1,1] <@ arraymultirange(arrayrange(array[1,2], array[2,1]));
 ...
 create type two_ints_range as range (subtype = two_ints);
  ...
  drop type two_ints cascade;
   set enable_sort = off;
  -- try to make it pick a hash setop implementation  select '{(2,5)}'::cashmultirange except select '{(5,6)}'::cashmultirange;
  reset enable_sort;
 ...
 select * from mr_outparam_succeed3(int4multirange(int4range(1,2)));
  ...
  create function mr_polymorphic(i anyrange) returns anymultirange as $$ begin return multirange($1);
 end;
 $$ language plpgsql;
