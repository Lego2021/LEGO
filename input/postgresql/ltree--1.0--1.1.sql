/* contrib/ltree/ltree--1.0--1.1.sql */  \echo Use "ALTER EXTENSION ltree UPDATE TO '1.1'" to load this file. \quit  DO LANGUAGE plpgsql $$ DECLARE my_schema pg_catalog.text := pg_catalog.quote_ident(pg_catalog.current_schema());
 old_path pg_catalog.text := pg_catalog.current_setting('search_path');
 BEGIN PERFORM pg_catalog.set_config('search_path', 'pg_catalog, pg_temp', true);
  UPDATE pg_catalog.pg_proc SET proargtypes = pg_catalog.array_to_string(newtypes::pg_catalog.oid[], ' ')::pg_catalog.oidvector, pronargs = pg_catalog.array_length(newtypes, 1) FROM (VALUES (NULL::pg_catalog.text, NULL::pg_catalog.text[]), -- establish column types ('ltree_consistent(internal,internal,int2,oid,internal)', '{internal,SCH.ltree,int2,oid,internal}'), ('ltree_same(internal,internal,internal)', '{SCH.ltree_gist,SCH.ltree_gist,internal}'), ('_ltree_consistent(internal,internal,int2,oid,internal)', '{internal,SCH._ltree,int2,oid,internal}'), ('_ltree_same(internal,internal,internal)', '{SCH.ltree_gist,SCH.ltree_gist,internal}') ) AS update_data (oldproc, newtypestext), LATERAL ( SELECT array_agg(replace(typ, 'SCH', my_schema)::regtype) as newtypes FROM unnest(newtypestext) typ ) ls WHERE oid = to_regprocedure(my_schema || '.' || replace(oldproc, 'SCH', my_schema));
  UPDATE pg_catalog.pg_proc SET prorettype = (my_schema || '.ltree_gist')::pg_catalog.regtype WHERE oid = pg_catalog.to_regprocedure(my_schema || '.ltree_union(internal,internal)');
  UPDATE pg_catalog.pg_proc SET prorettype = (my_schema || '.ltree_gist')::pg_catalog.regtype WHERE oid = pg_catalog.to_regprocedure(my_schema || '._ltree_union(internal,internal)');
