 ...
 \d+ gin_relopts_test  CREATE INDEX hash_i4_index ON hash_i4_heap USING hash (random int4_ops);
 ...
 INSERT INTO func_index_heap VALUES('QWERTY');
  \d func_index_heap \d func_index_index   DROP TABLE func_index_heap;
 CREATE TABLE func_index_heap (f1 text, f2 text);
 ...
 INSERT INTO func_index_heap VALUES('ABCD', 'EF');
 INSERT INTO func_index_heap VALUES('QWERTY');
  \d func_index_heap \d func_index_index  create index on func_index_heap ((f1 || f2), (row(f1, f2)));
 ...
 VACUUM FULL concur_heap;
 \d concur_heap REINDEX TABLE concur_heap;
 \d concur_heap  CREATE TEMP TABLE concur_temp (f1 int, f2 text) ON COMMIT PRESERVE ROWS;
 ...
 CREATE INDEX CONCURRENTLY concur_temp_ind ON concur_temp(f1);
 COMMIT;
 CREATE TEMP TABLE concur_temp (f1 int, f2 text) ON COMMIT DELETE ROWS;
 INSERT INTO concur_temp VALUES (1, 'foo'), (2, 'bar');
 CREATE INDEX CONCURRENTLY concur_temp_ind ON concur_temp(f1);
 DROP INDEX CONCURRENTLY concur_temp_ind;
 DROP TABLE concur_temp;
  DROP INDEX CONCURRENTLY "concur_index2";
                -- works DROP INDEX CONCURRENTLY IF EXISTS "concur_index2";
        -- notice  DROP INDEX CONCURRENTLY "concur_index2", "concur_index3";
 BEGIN;
 ...
 DROP INDEX CONCURRENTLY "concur_heap_expr_idx";
  \d concur_heap  DROP TABLE concur_heap;
   CREATE TABLE cwi_test( a int , b varchar(10), c char);
   INSERT INTO cwi_test VALUES(1, 2), (3, 4), (5, 6);
  CREATE UNIQUE INDEX cwi_uniq_idx ON cwi_test(a , b);
 ALTER TABLE cwi_test ADD primary key USING INDEX cwi_uniq_idx;
  \d cwi_test \d cwi_uniq_idx  CREATE UNIQUE INDEX cwi_uniq2_idx ON cwi_test(b , a);
 ALTER TABLE cwi_test DROP CONSTRAINT cwi_uniq_idx, ADD CONSTRAINT cwi_replaced_pkey PRIMARY KEY USING INDEX cwi_uniq2_idx;
  \d cwi_test \d cwi_replaced_pkey  DROP INDEX cwi_replaced_pkey;
    -- Should fail;
 a constraint depends on it  CREATE UNIQUE INDEX cwi_uniq3_idx ON cwi_test(a desc);
 ALTER TABLE cwi_test ADD UNIQUE USING INDEX cwi_uniq3_idx;
  -- fail CREATE UNIQUE INDEX cwi_uniq4_idx ON cwi_test(b collate "POSIX");
 ALTER TABLE cwi_test ADD UNIQUE USING INDEX cwi_uniq4_idx;
  -- fail  DROP TABLE cwi_test;
 ...
 SELECT count(*) FROM dupindexcols WHERE f1 BETWEEN 'WA' AND 'ZZZ' and id < 1000 and f1 ~<~ 'YX';
   vacuum tenk1;
        -- ensure we get consistent plans here  explain (costs off) SELECT unique1 FROM tenk1 WHERE unique1 IN (1,42,7) ORDER BY unique1;
 ...
 -- notice ALTER TABLE concur_reindex_tab ADD COLUMN c2 text;
 -- add toast index CREATE UNIQUE INDEX concur_reindex_ind1 ON concur_reindex_tab(c1);
 ...
 REINDEX INDEX CONCURRENTLY  concur_reindex_tab3_c2_excl;
  ...
  -- succeeds with warning INSERT INTO concur_reindex_tab3 VALUES  (4, '[2,4]');
 ...
 SELECT indexrelid::regclass, indisclustered FROM pg_index WHERE indrelid = 'concur_clustered'::regclass;
 ...
 SELECT indexrelid::regclass, indisreplident FROM pg_index WHERE indrelid = 'concur_replident'::regclass;
 ...
 -- error REINDEX TABLE concur_reindex_part_index_10;
 -- error REINDEX TABLE CONCURRENTLY concur_reindex_part_index_10;
 -- error BEGIN;
 REINDEX INDEX concur_reindex_part_index;
 ROLLBACK;
 CREATE OR REPLACE FUNCTION create_relfilenode_part(relname text, indname text) RETURNS VOID AS $func$ BEGIN EXECUTE format(' CREATE TABLE %I AS SELECT oid, relname, relfilenode, relkind, reltoastrelid FROM pg_class WHERE oid IN (SELECT relid FROM pg_partition_tree(''%I''));
', relname, indname);
 END $func$ LANGUAGE plpgsql;
 CREATE OR REPLACE FUNCTION compare_relfilenode_part(tabname text) RETURNS TABLE (relname name, relkind "char", state text) AS $func$ BEGIN RETURN QUERY EXECUTE format( 'SELECT  b.relname, b.relkind, CASE WHEN a.relfilenode = b.relfilenode THEN ''relfilenode is unchanged'' ELSE ''relfilenode has changed'' END FROM %I b JOIN pg_class a ON b.relname = a.relname ORDER BY 1;
', tabname);
 END $func$ LANGUAGE plpgsql;
 ...
 -- error REINDEX INDEX concur_reindex_part_10;
 ...
 -- no catalog index REINDEX TABLE CONCURRENTLY pg_toast.pg_toast_1260;
 -- no catalog toast table REINDEX INDEX CONCURRENTLY pg_toast.pg_toast_1260_index;
 -- no catalog toast index REINDEX SYSTEM CONCURRENTLY postgres;
 -- not allowed for SYSTEM REINDEX SCHEMA CONCURRENTLY pg_catalog;
  \d concur_reindex_tab DROP MATERIALIZED VIEW concur_reindex_matview;
 DROP TABLE concur_reindex_tab, concur_reindex_tab2, concur_reindex_tab3;
  CREATE TABLE concur_reindex_tab4 (c1 int);
 INSERT INTO concur_reindex_tab4 VALUES (1), (1), (2);
 CREATE UNIQUE INDEX CONCURRENTLY concur_reindex_ind5 ON concur_reindex_tab4 (c1);
 REINDEX INDEX CONCURRENTLY concur_reindex_ind5;
 \d concur_reindex_tab4 DROP INDEX concur_reindex_ind5_ccnew;
 DELETE FROM concur_reindex_tab4 WHERE c1 = 1;
 REINDEX TABLE CONCURRENTLY concur_reindex_tab4;
 \d concur_reindex_tab4 REINDEX INDEX CONCURRENTLY concur_reindex_ind5;
 \d concur_reindex_tab4 DROP TABLE concur_reindex_tab4;
 ...
 SELECT starelid::regclass, count(*) FROM pg_statistic WHERE starelid IN ( 'concur_exprs_index_expr'::regclass, 'concur_exprs_index_pred'::regclass, 'concur_exprs_index_pred_2'::regclass) GROUP BY starelid ORDER BY starelid::regclass::text;
 ...
 SELECT pg_get_indexdef('concur_exprs_index_pred_2'::regclass);
 SELECT starelid::regclass, count(*) FROM pg_statistic WHERE starelid IN ( 'concur_exprs_index_expr'::regclass, 'concur_exprs_index_pred'::regclass, 'concur_exprs_index_pred_2'::regclass) GROUP BY starelid ORDER BY starelid::regclass::text;
 SELECT attrelid::regclass, attnum, attstattarget FROM pg_attribute WHERE attrelid IN ( 'concur_exprs_index_expr'::regclass, 'concur_exprs_index_pred'::regclass, 'concur_exprs_index_pred_2'::regclass) ORDER BY attrelid::regclass::text, attnum;
 DROP TABLE concur_exprs_tab;
  CREATE TEMP TABLE concur_temp_tab_1 (c1 int, c2 text) ON COMMIT PRESERVE ROWS;
 ...
 REINDEX INDEX CONCURRENTLY concur_temp_ind_1;
 COMMIT;
 CREATE TEMP TABLE concur_temp_tab_2 (c1 int, c2 text) ON COMMIT DELETE ROWS;
 CREATE INDEX concur_temp_ind_2 ON concur_temp_tab_2(c2);
 REINDEX TABLE CONCURRENTLY concur_temp_tab_2;
 REINDEX INDEX CONCURRENTLY concur_temp_ind_2;
 BEGIN;
 CREATE TEMP TABLE concur_temp_tab_3 (c1 int, c2 text) ON COMMIT PRESERVE ROWS;
 ...
 SELECT pg_my_temp_schema()::regnamespace as temp_schema_name \gset REINDEX SCHEMA CONCURRENTLY :temp_schema_name;
 SELECT  b.relname, b.relkind, CASE WHEN a.relfilenode = b.relfilenode THEN 'relfilenode is unchanged' ELSE 'relfilenode has changed' END FROM reindex_temp_before b JOIN pg_class a ON b.oid = a.oid ORDER BY 1;
 DROP TABLE concur_temp_tab_1, concur_temp_tab_2, reindex_temp_before;
  REINDEX SCHEMA schema_to_reindex;
 -- failure, schema does not exist CREATE SCHEMA schema_to_reindex;
 ...
 BEGIN;
 REINDEX SCHEMA schema_to_reindex;
 -- failure, cannot run in a transaction END;
