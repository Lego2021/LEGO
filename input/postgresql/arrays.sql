 CREATE TABLE arrtest ( a             int2[], b             int4[][][], c             name[], d            text[][], e             float8[], f            char(5)[], g            varchar(5)[] );
  ...
  -- error, type mismatch  INSERT INTO arrtest (b[1:2]) VALUES(now());
  -- error, type mismatch  SELECT * FROM arrtest;
 ...
 UPDATE arrtest_s SET a[:] = '{23, 24, 25}';
  -- fail, too small INSERT INTO arrtest_s VALUES(NULL, NULL);
 UPDATE arrtest_s SET a[:] = '{11, 12, 13, 14, 15}';
  -- fail, no good with null  SELECT f1[0:1] FROM POINT_TBL;
 ...
 -- f select 'foo' not like any (array['%a', '%b']);
 -- t select 'foo' not like all (array['%a', '%o']);
 -- f select 'foo' ilike any (array['%A', '%O']);
 -- t select 'foo' ilike all (array['F%', '%O']);
 -- t   select '{{1,{2}},{2,3}}'::text[];
 select '{{},{}}'::text[];
 ...
 drop type textandtext;
  ...
  SELECT op, width_bucket(op, ARRAY[1, 3, 9, 'NaN', 'NaN']::float8[]) AS wb FROM (VALUES (-5.2::float8), (4::float8), (77::float8), ('NaN'::float8) ) v(op);
