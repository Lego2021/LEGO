CREATE EXTENSION tsm_system_rows;
 ...
 SELECT count(*) FROM test_tablesample TABLESAMPLE system_rows (100);
  EXPLAIN (COSTS OFF) SELECT id FROM test_tablesample TABLESAMPLE system_rows (-1);
  ...
  EXPLAIN (COSTS OFF) SELECT * FROM (VALUES (0),(10),(100)) v(nrows), LATERAL (SELECT count(*) FROM test_tablesample TABLESAMPLE system_rows (nrows)) ss;
  SELECT * FROM (VALUES (0),(10),(100)) v(nrows), LATERAL (SELECT count(*) FROM test_tablesample TABLESAMPLE system_rows (nrows)) ss;
  CREATE VIEW vv AS SELECT count(*) FROM test_tablesample TABLESAMPLE system_rows (20);
  SELECT * FROM vv;
