CREATE TEMP TABLE x ( a serial, b int, c text not null default 'stuff', d text, e text ) ;
 ...
 x,45,80,90 x,\x,\\x,\\\x x,\,,\\\,,\\ \.  COPY x from stdin WITH DELIMITER AS ';
' NULL AS '';
 3000;
;
c;
;
 ...
 \d+ check_con_tbl copy check_con_tbl from stdin;
 ...
 -- fail test1 \.  CREATE FUNCTION fun_instead_of_insert_tbl() RETURNS trigger AS $$ BEGIN INSERT INTO instead_of_insert_tbl (name) VALUES (NEW.str);
 RETURN NULL;
 END;
 $$ LANGUAGE plpgsql;
