/* src/test/modules/test_pg_dump/test_pg_dump--1.0.sql */  \echo Use "CREATE EXTENSION test_pg_dump" to load this file. \quit  CREATE TABLE regress_pg_dump_table ( col1 serial, col2 int check (col2 > 0) );
  CREATE SEQUENCE regress_pg_dump_seq;
  CREATE SEQUENCE regress_seq_dumpable;
 SELECT pg_catalog.pg_extension_config_dump('regress_seq_dumpable', '');
  CREATE TABLE regress_table_dumpable ( col1 int check (col1 > 0) );
 ...
 GRANT ALL ON FUNCTION wgo_then_no_access() TO pg_signal_backend WITH GRANT OPTION;
 ...
 REVOKE GRANT OPTION FOR SELECT ON SEQUENCE wgo_then_regular FROM pg_signal_backend;
  ...
  CREATE TABLE regress_pg_dump_schema.test_table ( col1 int, col2 int check (col2 > 0) );
 ...
 GRANT USAGE ON TYPE regress_pg_dump_schema.test_type TO regress_dump_test_role;
  CREATE FUNCTION regress_pg_dump_schema.test_func () RETURNS int AS 'SELECT 1;
' LANGUAGE SQL;
