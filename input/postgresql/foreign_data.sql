 ...
 \dew  CREATE FOREIGN DATA WRAPPER foo;
 -- duplicate DROP FOREIGN DATA WRAPPER foo;
 CREATE FOREIGN DATA WRAPPER foo OPTIONS (testing '1');
 \dew+  DROP FOREIGN DATA WRAPPER foo;
 ...
 \dew+  CREATE FUNCTION invalid_fdw_handler() RETURNS int LANGUAGE SQL AS 'SELECT 1;
';
 ...
 \dew+  ALTER FOREIGN DATA WRAPPER foo OPTIONS (a '1', b '2');
 ...
 \dew+  ALTER FOREIGN DATA WRAPPER foo OPTIONS (DROP a, SET b '3', ADD c '4');
 \dew+  ALTER FOREIGN DATA WRAPPER foo OPTIONS (a '2');
 ALTER FOREIGN DATA WRAPPER foo OPTIONS (b '4');
             -- ERROR \dew+  SET ROLE regress_test_role;
 ALTER FOREIGN DATA WRAPPER foo OPTIONS (ADD d '5');
         -- ERROR SET ROLE regress_test_role_super;
 ALTER FOREIGN DATA WRAPPER foo OPTIONS (ADD d '5');
 \dew+  ALTER FOREIGN DATA WRAPPER foo OWNER TO regress_test_role;
 ...
 ALTER FOREIGN DATA WRAPPER foo OPTIONS (ADD e '6');
         -- ERROR RESET ROLE;
 \dew+  ALTER FOREIGN DATA WRAPPER foo RENAME TO foo1;
 \dew+ ALTER FOREIGN DATA WRAPPER foo1 RENAME TO foo;
 ...
 \dew+  DROP ROLE regress_test_role_super;
                          -- ERROR SET ROLE regress_test_role_super;
 ...
 -- NOTICE \dew+ \des+ \deu+ DROP FOREIGN DATA WRAPPER foo;
                              -- ERROR SET ROLE regress_test_role;
 DROP FOREIGN DATA WRAPPER foo CASCADE;
                      -- ERROR RESET ROLE;
 DROP FOREIGN DATA WRAPPER foo CASCADE;
 \dew+ \des+ \deu+  CREATE SERVER s1 FOREIGN DATA WRAPPER foo;
 ...
 \des+ SET ROLE regress_test_role;
 ...
 CREATE SERVER t1 FOREIGN DATA WRAPPER foo;
 RESET ROLE;
 \des+  REVOKE USAGE ON FOREIGN DATA WRAPPER foo FROM regress_test_role;
 ...
 DROP ROLE regress_test_indirect;
                            -- ERROR \des+  ALTER SERVER s8 RENAME to s8new;
 \des+ ALTER SERVER s8new RENAME to s8;
 ...
 RESET ROLE;
 \des ALTER SERVER s2 OWNER TO regress_test_role;
 SET ROLE regress_test_role;
 DROP SERVER s2;
 RESET ROLE;
 \des CREATE USER MAPPING FOR current_user SERVER s3;
 \deu DROP SERVER s3;
                                             -- ERROR DROP SERVER s3 CASCADE;
 \des \deu  CREATE USER MAPPING FOR regress_test_missing_role SERVER s1;
 ...
 RESET ROLE;
 \deu  ALTER USER MAPPING FOR regress_test_missing_role SERVER s4 OPTIONS (gotcha 'true');
 ...
 RESET ROLE;
 \deu+  DROP USER MAPPING FOR regress_test_missing_role SERVER s4;
 ...
 DROP USER MAPPING FOR public SERVER s8;
                     -- ERROR RESET ROLE;
 DROP SERVER s7;
 \deu  CREATE SCHEMA foreign_schema;
 ...
 -- ERROR CREATE TABLE ref_table (id integer PRIMARY KEY);
 ...
 \d+ ft1 \det+ CREATE INDEX id_ft1_c2 ON ft1 (c2);
                             -- ERROR SELECT * FROM ft1;
                                              -- ERROR EXPLAIN SELECT * FROM ft1;
                                      -- ERROR  CREATE TABLE lt1 (a INT) PARTITION BY RANGE (a);
 CREATE FOREIGN TABLE ft_part1 PARTITION OF lt1 FOR VALUES FROM (0) TO (1000) SERVER s0;
 CREATE INDEX ON lt1 (a);
                              -- skips partition CREATE UNIQUE INDEX ON lt1 (a);
                                 -- ERROR ALTER TABLE lt1 ADD PRIMARY KEY (a);
                            -- ERROR DROP TABLE lt1;
 ...
 \d+ ft1 CREATE TABLE use_ft1_column_type (x ft1);
 ALTER FOREIGN TABLE ft1 ALTER COLUMN c8 SET DATA TYPE integer;
    -- ERROR DROP TABLE use_ft1_column_type;
 ...
 \d foreign_schema.foreign_table_1  ALTER FOREIGN TABLE IF EXISTS doesnt_exist_ft1 ADD COLUMN c4 integer;
 ...
 \d+ fd_pt1 \d+ ft2 DROP FOREIGN TABLE ft2;
 \d+ fd_pt1 CREATE FOREIGN TABLE ft2 ( c1 integer NOT NULL, c2 text, c3 date ) SERVER s0 OPTIONS (delimiter ',', quote '"', "be quoted" 'value');
 \d+ ft2 ALTER FOREIGN TABLE ft2 INHERIT fd_pt1;
 \d+ fd_pt1 \d+ ft2 CREATE TABLE ct3() INHERITS(ft2);
 CREATE FOREIGN TABLE ft3 ( c1 integer NOT NULL, c2 text, c3 date ) INHERITS(ft2) SERVER s0;
 \d+ ft2 \d+ ct3 \d+ ft3  ALTER TABLE fd_pt1 ADD COLUMN c4 integer;
 ...
 \d+ fd_pt1 \d+ ft2 \d+ ct3 \d+ ft3  ALTER TABLE fd_pt1 ALTER COLUMN c4 SET DEFAULT 0;
 ...
 ALTER TABLE fd_pt1 ALTER COLUMN c8 TYPE char(10) USING '0';
        -- ERROR ALTER TABLE fd_pt1 ALTER COLUMN c8 TYPE char(10);
 ...
 \d+ fd_pt1 \d+ ft2  ALTER TABLE fd_pt1 DROP COLUMN c4;
 ...
 \d+ fd_pt1 \d+ ft2  ALTER TABLE fd_pt1 ADD CONSTRAINT fd_pt1chk1 CHECK (c1 > 0) NO INHERIT;
 ...
 ALTER FOREIGN TABLE ft2 INHERIT fd_pt1;
 \d+ fd_pt1 \d+ ft2  ALTER TABLE fd_pt1 DROP CONSTRAINT fd_pt1chk1 CASCADE;
 ...
 \d+ fd_pt1 \d+ ft2 ALTER TABLE fd_pt1 VALIDATE CONSTRAINT fd_pt1chk3;
 \d+ fd_pt1 \d+ ft2  ALTER TABLE fd_pt1 RENAME COLUMN c1 TO f1;
 ...
 \d+ fd_pt1 \d+ ft2  TRUNCATE ft2;
  -- ERROR TRUNCATE fd_pt1;
  -- ERROR  DROP TABLE fd_pt1 CASCADE;
 ...
 -- ERROR IMPORT FOREIGN SCHEMA s1 EXCEPT (t1, t2) FROM SERVER s9 INTO public OPTIONS (option1 'value1', option2 'value2');
 -- ERROR  DROP FOREIGN TABLE no_table;
 ...
 \d+ fd_pt2 \d+ fd_pt2_1  DROP FOREIGN TABLE fd_pt2_1;
 ...
 \d+ fd_pt2_1 ALTER TABLE fd_pt2 ATTACH PARTITION fd_pt2_1 FOR VALUES IN (1);
       -- ERROR  DROP FOREIGN TABLE fd_pt2_1;
 \d+ fd_pt2 CREATE FOREIGN TABLE fd_pt2_1 ( c1 integer NOT NULL, c2 text, c3 date ) SERVER s0 OPTIONS (delimiter ',', quote '"', "be quoted" 'value');
 \d+ fd_pt2_1 ALTER TABLE fd_pt2 ATTACH PARTITION fd_pt2_1 FOR VALUES IN (1);
 \d+ fd_pt2 \d+ fd_pt2_1  ALTER TABLE fd_pt2_1 ADD c4 char;
  ALTER TABLE fd_pt2_1 ALTER c3 SET NOT NULL;
 ALTER TABLE fd_pt2_1 ADD CONSTRAINT p21chk CHECK (c2 <> '');
 \d+ fd_pt2 \d+ fd_pt2_1  ALTER TABLE fd_pt2_1 ALTER c1 DROP NOT NULL;
  ALTER TABLE fd_pt2 DETACH PARTITION fd_pt2_1;
 ALTER TABLE fd_pt2 ALTER c2 SET NOT NULL;
 \d+ fd_pt2 \d+ fd_pt2_1 ALTER TABLE fd_pt2 ATTACH PARTITION fd_pt2_1 FOR VALUES IN (1);
