...
' LANGUAGE SQL;
 ...
 select * from rngfunct(1) with ordinality as z(a,b,ord) where b > 100;
   -- ordinal 2, not 1 select a,b,ord from rngfunct(1) with ordinality as z(a,b,ord);
...
' LANGUAGE plpgsql;
 ...
 SELECT dup(22);
  ...
  CREATE FUNCTION dup (f1 anycompatible, f2 anycompatiblearray, f3 out anycompatible, f4 out anycompatiblearray) AS 'select $1, $2' LANGUAGE sql;
 ...
 SELECT *, pg_typeof(f3), pg_typeof(f4) FROM dup(22, array[44::bigint]);
  ...
  CREATE FUNCTION dup (f1 anycompatiblerange, f2 out anycompatible, f3 out anycompatiblearray, f4 out anycompatiblerange) AS 'select lower($1), array[lower($1), upper($1)], $1' LANGUAGE sql;
 SELECT dup(int4range(4,7));
 SELECT dup(numrange(4,7));
 SELECT dup(textrange('aaa', 'bbb'));
  DROP FUNCTION dup(f1 anycompatiblerange);
  CREATE FUNCTION bad (f1 anyarray, out f2 anycompatible, out f3 anycompatiblearray) AS 'select $1, array[$1,$1]' LANGUAGE sql;
 ...
 -- fail select * from array_to_set(array['one', 'two']) as t(f1 numeric(4,2),f2 text);
 ...
 -- fail  drop function testrngfunc();
 ...
 -- fail  drop function testrngfunc();
  ...
  create function testrngfunc() returns rngfunc_type as $$ select 7.136178319899999964, 7.136178319899999964;
 $$ language sql immutable;
  explain (verbose, costs off) select testrngfunc();
 select testrngfunc();
 explain (verbose, costs off) select * from testrngfunc();
 select * from testrngfunc();
  create or replace function testrngfunc() returns rngfunc_type as $$ select 7.136178319899999964, 7.136178319899999964;
 ...
 select * from testrngfunc();
  ...
  create function testrngfunc() returns setof rngfunc_type as $$ select 7.136178319899999964, 7.136178319899999964;
 $$ language sql immutable;
  explain (verbose, costs off) select testrngfunc();
 select testrngfunc();
 explain (verbose, costs off) select * from testrngfunc();
 select * from testrngfunc();
  create or replace function testrngfunc() returns setof rngfunc_type as $$ select 7.136178319899999964, 7.136178319899999964;
 $$ language sql volatile;
  explain (verbose, costs off) select testrngfunc();
 select testrngfunc();
 explain (verbose, costs off) select * from testrngfunc();
 select * from testrngfunc();
  create or replace function testrngfunc() returns setof rngfunc_type as $$ select 1, 2 union select 3, 4 order by 1;
 $$ language sql immutable;
  explain (verbose, costs off) select testrngfunc();
 select testrngfunc();
 explain (verbose, costs off) select * from testrngfunc();
 select * from testrngfunc();
  select * from testrngfunc() as t(f1 int8,f2 int8);
  -- fail, composite result select * from pg_get_keywords() as t(f1 int8,f2 int8);
  -- fail, OUT params select * from sin(3) as t(f1 int8,f2 int8);
 ...
 alter table users drop column todrop;
  create or replace function get_first_user() returns users as $$ SELECT * FROM users ORDER BY userid LIMIT 1;
 $$ language sql stable;
  SELECT get_first_user();
 SELECT * FROM get_first_user();
  create or replace function get_users() returns setof users as $$ SELECT * FROM users ORDER BY userid;
 $$ language sql stable;
  SELECT get_users();
 SELECT * FROM get_users();
 SELECT * FROM get_users() WITH ORDINALITY;
   -- make sure ordinality copes  SELECT * FROM ROWS FROM(generate_series(10,11), get_users()) WITH ORDINALITY;
 ...
 select * from usersview;
  -- expect clean failure rollback;
 alter table users alter column seq type numeric;
 select * from usersview;
  -- expect clean failure  drop view usersview;
 drop function get_first_user();
 drop function get_users();
 drop table users;
   create or replace function rngfuncbar() returns setof text as $$ select 'foo'::varchar union all select 'bar'::varchar ;
 $$ language sql stable;
  select rngfuncbar();
 ...
 explain (verbose, costs off) select * from rngfuncbar();
  ...
  select * from rngfuncbar();
  -- fail  create or replace function rngfuncbar(out integer, out numeric) as $$ select (1, 2.1, 3) $$ language sql;
  select * from rngfuncbar();
  -- fail  drop function rngfuncbar();
